...
.DEFAULT_GOAL := help
.PHONY = fmt vet all install uninstall clean deps update test init vendor tidy mod-init mod-update mod-download mod-vendor help
...
perfecto:
	@echo "[36;1mBuilding perfecto…[0m"
	@go build $(VERBOSE_FLAG) -ldflags="-X main.gitrev=$(GITREV)" perfecto.go
...
ifdef COVERAGE_FILE ## Save coverage data into file (String)
	@go test $(VERBOSE_FLAG) -covermode=count -coverprofile=$(COVERAGE_FILE) ./check ./spec
else
	@go test $(VERBOSE_FLAG) -covermode=count ./check ./spec
endif
